...
// 可能抛出异常
function APIObject(opts, requestWrapper) {
	let self = this instanceof APIObject ? this : Object.create(APIObject.prototype);
	if (!util.isObjectOrNull(opts)) {
		throw new Error('Options must be an object or null.');
	}
	//todo 判断request是否具有对应接口
	if (util.isFunction(requestWrapper)) {
		self.requestWrapper = requestWrapper;
	} else {
		throw new Error('RequestWrapper must be a funnction.');
	}
	...
	if (util.isObject(opts)) {
		...
		if (self.method === APIObject.POST) {
			...
			if (opts.type != null && opts.type != APIObject.JSON && opts.type !=
				APIObject.FORM) {
				throw new Error(
					`Unkown property value: ${opts.type}. Type must be JSON or FORM.`);
			}
	...
	if (!util.isFunction(this.get)) {
		...
		APIObject.prototype.get = function () {
			...
			if (callback && !flag) {
				...
				return this.requestWrapper(this.method, requestURL, opts);
			} else if (!callback && !flag) {
				return this.requestWrapper(this.method, requestURL);
			}
		};
	}
	...
	if (!util.isFunction(this.post)) {
		APIObject.prototype.post = function () {
			...
			let bodyType = APIObject.JSON;
			let body = '';

			// 带参,是查询字符串
			if (typeof queryString === 'string') {
				// url不能修改，不然对同一个api调用多次会导致url一直变长
				requestURL = this.url.indexOf('?') === -1 ? this.url + '?' +
					encodeURI(
						queryString) :
			...
			if (callback && !flag) {
				...
				return this.requestWrapper(this.method, requestURL, opts, bodyType);
			} else if (!callback && !flag) {
				return this.requestWrapper(this.method, requestURL, body, bodyType);
			}
